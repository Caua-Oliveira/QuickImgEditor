...
class ImageProcessor:
    ...
    @classmethod
    def remove_background(cls, image: Image.Image) -> Image.Image:
...
class TrayManager:
    ...
    def create_icon(self):
        ...
        # Ensure that "icon.png" exists in the same directory or provide the full path.
    ...
    def show_options(self, icon, item):
        # This method now opens the options page in the main application.
...
class ImageEditorApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Enhanced Image Editor")
    ...
    def remove_background(self):
        if self.base_image:
            try:
                self.base_image = ImageProcessor.remove_background(self.base_image)
                ...
                logging.warning(f"Failed to remove background")
    ...
    def load_theme(self):
        try:
            if getattr(sys, 'frozen', False):
                theme_path = os.path.join(sys._MEIPASS, 'theme.json')
                print("app")
            else:
    ...
    def setup_bindings(self):
        ...
        keyboard.add_hotkey('ctrl+shift+b', self.toggle_visibility)
    ...
    def create_processing_buttons(self):
        ...
        btn = ctk.CTkButton(processing_frame, text="Remove Background", command=self.remove_background)
    ...
    def open_options_page(self):
        # If the options window is already open, just focus it.
        ...
        # Example: Add a simple label and a dummy option.
        label = ctk.CTkLabel(self.options_window, text="Options Page", font=("Arial", 20))
        label.pack(pady=20)

        # Example: A dummy checkbox option.
        dummy_var = tk.BooleanVar(value=False)
        checkbox = ctk.CTkCheckBox(self.options_window, text="Enable Feature", variable=dummy_var)
